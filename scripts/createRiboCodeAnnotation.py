...
def change_annotation(args):
    ...
    wanted_features = ["gene", "transcript", "exon", "CDS"]
    ...
    for key, val in gene_dict.items():
        ...
        # start + stop
        if val[3] == "+":
            rows.append(nTuple(region, "generated", "start_codon", start, start+2, val[3], val[4], val[5], val[6]))
            rows.append(nTuple(region, "generated", "stop_codon", stop-2, stop, val[3], val[4], val[5], val[6]))
        else:
            rows.append(nTuple(region, "generated", "start_codon", stop-2, stop, val[3], val[4], val[5], val[6]))
            rows.append(nTuple(region, "generated", "stop_codon", start, start+2, val[3], val[4], val[5], val[6]))
