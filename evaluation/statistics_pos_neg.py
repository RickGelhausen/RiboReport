...
def get_assosiated_genes(gene_dict, pred_predition_dict, label):
    ...
    score_list = []
    fn = 0
    #[print(k,v) for (k,v) in pred_predition_dict.items()]
    #print('\n')
    #[print(k,v) for (k,v) in gene_dict.items()]
    #print('\n')
    ...
    return (assosiated_genes_dict, pred_predition_sub_dict,
            assosiated_predictions_dict, fn, score_list)
...
def get_genes(gene_dict, df_overlap_score, tool):
    ...
    gene_predition_dict ={str(k[0]) + ':' + str(k[1]) + ':' +
                          k[2] + ':' + k[3]: gene_predition_temp_dict[k]
...
def get_prediction(tool_dict, df_overlap_score, tool):
    ...
    pred_predition_dict = {str(k[0]) + ':' + str(k[1]) + ':' +
                           k[2] + ':' + k[3]: pred_predition_temp_dict[k]
...
def assort_genes_with_predictions(ref_file, tools_file, df_overlap_score, tool, label):
    """This fuction computes TP, FN and FP for a given tool.
        ...
        assosiated_gene_list
            list of all genes found by the currently investigated tool
        score_list
            list contining for the current tool (score, overlap, label) for each assosiated gene
    ...
    #print(len(gene_predicted_dict))
    #print(list(gene_predicted_dict.keys()))
    #print(list(gene_predicted_dict.values())[0])
    #print('\n')
    ...
    #print(len(predition_overlap_dict))
    #print('\n')
    # assosiat each gene with just one prediciton. All othere genes
    assosiated_genes_dict, pred_predition_sub_list, assosiated_predictions_dict, no_genes_lost_prediction, score_list = get_assosiated_genes(gene_predicted_dict, predition_overlap_dict,label)
    #print(len(assosiated_genes_dict))
    #print('\n')
    ...
    if not len(assosiated_genes_dict):
        ...
        assorted_genes = len(assosiated_predictions_dict)
        not_assorted_genes = len(gene_dict) - len(assosiated_predictions_dict)

    if not len(pred_predition_sub_list):
        suboptimals = 0
    else:
        suboptimals = len(pred_predition_sub_list)

    assosiated_gene_list =  list(assosiated_genes_dict.keys())

    return assorted_genes, not_assorted_genes, suboptimals, assosiated_gene_list, score_list
...
def get_stat_for_tool(ref_file, tools_file, df_overlap_score, ref_neg_file, df_neg_overlap, df_fp_overlap, flag_subopt, flag_no_gene, tool, save_dir):
    ...
    """
    TP, FN, suboptimals_TP, assosiated_TP_list, score_list_pos = assort_genes_with_predictions(ref_file, tools_file, df_overlap_score, tool, '1')
    FP, TN, suboptimals_FP, assosiated_FP_list, score_list_neg = assort_genes_with_predictions(ref_neg_file, tools_file, df_neg_overlap, tool, '0')
    ...
    return stat_list, assosiated_TP_list
...
def call_bedtools_not_overlap(reference_path, tools_path, overlap_cutoff, overlap_file, score_cutoff):
    ...
    if os.stat(overlap_file).st_size == 0:
        print('**********************\nno prediction exitst that could not be assosiated with any gene\n**********************\n')
